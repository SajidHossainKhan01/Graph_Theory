...
public class EdmondsKarp {
    ...
    public static<T> ArrayList<T[]> bfs(HashMap<T, HashMap<T, Double>> residual, T source, T sink) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (u.equals(sink)) {
                ...
                while (prev.get(u) != null) {
                    path.add((T[]) new Object[]{prev.get(u), u});
                    u = prev.get(u);
                }
            ...
            for (T v : residual.get(u).keySet()) {
                if (!prev.containsKey(v) && residual.get(u).get(v) > 0) {
                    queue.add(v);
                    prev.put(v, u);
                }
            }
    ...
    public static<T> double edmondsKarp(HashMap<T, ArrayList<Tuple<T>>> graph) {
        ...
        double totalFlow = 0;
        while (true) {
            ArrayList<T[]> path = bfs(residual, (T) "s", (T) "t");
            if (path == null)
                break;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Graph 1: Maximum flow = " + edmondsKarp(graph1));
        System.out.println("Graph 2: Maximum flow = " + edmondsKarp(graph2));
