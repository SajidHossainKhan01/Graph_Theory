...
public class FordFulkerson {
    ...
    public static<T> ArrayList<T[]> dfs(HashMap<T, HashMap<T, Double>> residual, T u, T sink, ArrayList<T[]> path, HashSet<T> visited) {
        if (u.equals(sink))
            return path;
        visited.add(u);
        for (T v : residual.get(u).keySet()) {
            if (!visited.contains(v) && residual.get(u).get(v) > 0) {
                ...
                if (dfs(residual, v, sink, path, visited) != null)
                    return path;
    ...
    public static<T> double fordFulkerson(HashMap<T, ArrayList<Tuple<T>>> graph) {
        ...
        double totalFlow = 0;
        while (true) {
            ArrayList<T[]> path = dfs(residual, (T) "s", (T) "t", new ArrayList<>(), new HashSet<>());
            if (path == null)
                break;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Graph 1: Maximum flow = " + fordFulkerson(graph1));
        System.out.println("Graph 2: Maximum flow = " + fordFulkerson(graph2));
